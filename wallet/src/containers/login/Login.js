...
class Login extends React.Component {
  ...
  handleVerifyInvitationCode(event) {
    this.setState({ invitationCode: event.target.value});
    localStorage.setItem("invitationCode", event.target.value);
  }
  ...
  render() {
    ...
    let getTokenButtonText = 'Get Token';
    ...
    return (
      <div className={styles.Login}>
        <div className={styles.LoginInputContainer}>
          ...
          <br/>
          <div className={styles.Note}>
            You can click the button below to get 5 JLKN test Tokens per hour. 
          </div>
          <Button
            ...
            style={{marginTop: '10px'}}
            loading={buttonLoading}
            disabled={networkService.L1orL2 !== 'L2'}
          >
            {getTokenButtonText}
          </Button>
        </div>
